...
**番号付き `pass_fragments` を渡す案（paired）は2回検討し、2回とも見送った（ユーザー決定）。**
`full_dataset.csv` の `pass_fragments` には正しい対の番号が入っており、SAFE test split と
行対応も取れる（82,441行・`smiles` 完全一致・番号除去後も一致することを実測で確認済み）。
番号を振るのはモデルではなく前処理コードの仕事なので、正しい番号を使うこと自体は技術的に容易で、
再学習も不要（生成を1回追加で回すだけ）。それでも見送る理由は、番号が
「どの与断片同士が直接つながっているか」という**正解分子の部分構造情報**にあたり、
RFFMG・SAFE・PromptSMILES の3手法はいずれもこれを受け取らないためである。
入力情報量を4手法で厳密に揃えることを優先する。
